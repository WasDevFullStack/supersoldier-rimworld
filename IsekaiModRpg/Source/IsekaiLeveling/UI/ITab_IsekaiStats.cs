...
namespace IsekaiLeveling.UI
{
    ...
    public class ITab_IsekaiStats : ITab
    {
        ...
        private const float STAT_TAB_HEIGHT_BASE = 290f;  // 163 + ~92 gimmick + ~35 aura selector row
        ...
        private void FillTabVanilla()
        {
            ...
            // Aura display selector
            {
                Rect auraRect = new Rect(fullRect.x, curY, fullRect.width, 24f);
                if (Widgets.ButtonText(auraRect, "Isekai_AuraDisplay_Button".Translate(AuraButtonValueLabel(comp))))
                {
                    OpenAuraMenu(comp);
                }
                curY += 26f;
        ...
        private void DrawStatPanel(Rect fullRect, Pawn pawn, IsekaiComponent comp, float curY, float alpha)
        {
            ...
            using (var listing = new IsekaiListing(contentRect, alpha))
            {
                ...
                listing.Gap(4f);
                {
                    string current = AuraButtonValueLabel(comp);
                    Color auraBtnColor = new Color(0.118f, 0.075f, 0.063f); // #1E1310
                    if (listing.Button("Isekai_AuraDisplay_Button".Translate(current), 28f, auraBtnColor))
                    {
                        OpenAuraMenu(comp);
                    }
                }
        ...
        /// <summary>Label shown on the aura button — reflects the current mode and, for
        /// constellation mode, the chosen constellation (or the primary tree).</summary>
        private static string AuraButtonValueLabel(IsekaiComponent comp)
        {
            switch (comp.auraDisplayMode)
            {
                case AuraDisplayMode.DefaultFlame: return "Isekai_AuraDisplay_DefaultFlame".Translate();
                case AuraDisplayMode.None: return "Isekai_AuraDisplay_None".Translate();
                default:
                    ...
                    return string.IsNullOrEmpty(tree)
                        ? "Isekai_AuraDisplay_Constellation".Translate().ToString()
                        : tree;
            }
        }

        /// <summary>Builds the aura-selection FloatMenu: one entry per entered constellation
        /// (multiclass-aware), plus Default Flame and None.</summary>
        private static void OpenAuraMenu(IsekaiComponent comp)
        {
            var opts = new List<FloatMenuOption>();
            var trees = comp.passiveTree?.GetEnteredTrees();
            if (trees != null)
            {
                foreach (var tree in trees)
                {
                    string tc = tree.treeClass;
                    opts.Add(new FloatMenuOption(tc, () =>
                    {
                        comp.auraDisplayMode = AuraDisplayMode.Constellation;
                        comp.auraConstellation = tc;
                    }));
                }
            }
            opts.Add(new FloatMenuOption("Isekai_AuraDisplay_DefaultFlame".Translate(),
                () => comp.auraDisplayMode = AuraDisplayMode.DefaultFlame));
            opts.Add(new FloatMenuOption("Isekai_AuraDisplay_None".Translate(),
                () => comp.auraDisplayMode = AuraDisplayMode.None));
