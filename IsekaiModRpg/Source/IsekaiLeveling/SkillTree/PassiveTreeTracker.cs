...
namespace IsekaiLeveling.SkillTree
{
    ...
    public class PassiveTreeTracker : IExposable
    {
        ...
        /// <summary>All constellation trees this pawn has entered (supports multiclass).</summary>
        public List<PassiveTreeDef> GetEnteredTrees()
        {
            var result = new List<PassiveTreeDef>();
            foreach (var tree in DefDatabase<PassiveTreeDef>.AllDefs)
                if (HasEnteredTree(tree)) result.Add(tree);
            return result;
        }

        /// <summary>True if the pawn has entered the tree with the given treeClass name.</summary>
        public bool HasEnteredTreeByName(string treeClass)
        {
            if (string.IsNullOrEmpty(treeClass)) return false;
            foreach (var tree in DefDatabase<PassiveTreeDef>.AllDefs)
                if (tree.treeClass == treeClass) return HasEnteredTree(tree);
            return false;
        }
