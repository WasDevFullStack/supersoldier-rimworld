...
namespace IsekaiLeveling.Forge
{
    ...
    public static class ForgeUtility
    {
        ...
        private static void ResolveDefsIfNeeded()
        {
            ...
            _component = ResolveRefinementComponent();
            _defsResolved = true;
        }

        /// <summary>
        /// Resolves the component ThingDef used by refinement from mod settings, so packs that
        /// remove/disable industrial components (e.g. Medieval Overhaul) can point refinement at
        /// their own component. Falls back to ComponentIndustrial if the configured def is
        /// missing or unset, so a typo never bricks refinement.
        /// </summary>
        private static ThingDef ResolveRefinementComponent()
        {
            string name = IsekaiLevelingSettings.RefinementComponentDefName;
        ...
        /// <summary>The component ThingDef refinement currently consumes (settings-driven).</summary>
        public static ThingDef RefinementComponent
        {
            get
            {
                ResolveDefsIfNeeded();
        ...
        /// <summary>
        /// Re-resolve the component def after the player changes the setting at runtime, so the
        /// new component takes effect without a restart.
        /// </summary>
        public static void RefreshRefinementComponent()
        {
            _component = ResolveRefinementComponent();
        }
