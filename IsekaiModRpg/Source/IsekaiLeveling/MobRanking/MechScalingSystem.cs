...
namespace IsekaiLeveling.MobRanking
{
    ...
    public static class MechScalingSystem
    {
        ...
        /// <summary>
        /// Shooting-accuracy offset for a player mech's built-in weaponry, from the
        /// mechanitor's DEX. Mechs previously gained only melee benefits — this lets ranged
        /// combat mechs (e.g. Militors) actually improve as the mechanitor invests in DEX.
        /// </summary>
        public static float GetMechShootingOffset(Pawn mech)
        {
            var mechComp = GetMechanitorComp(mech);
            if (mechComp?.stats == null) return 0f;

            int dex = mechComp.stats.dexterity;
            return Math.Max(0, dex - 5) * IsekaiLevelingSettings.DEX_ShootingAccuracy;
        }
    }
    ...
    public static class MechStatScaling_Patch
    {
        ...
        public static void Postfix(ref float __result, StatRequest req, StatDef ___stat)
        {
            try
            {
                ...
                // Apply shooting-accuracy scaling for ranged combat mechs (from mechanitor DEX)
                else if (___stat == StatDefOf.ShootingAccuracyPawn)
                {
                    __result += MechScalingSystem.GetMechShootingOffset(pawn);
                }
                // Apply armor scaling
    ...
    public static class MechHealthScaling_Patch
    {
        ...
        public static void Postfix(ref float __result, Pawn pawn)
        {
            try
            {
                ...
                __result *= mult;
                // Whole-number body part HP — fractional HP corrupts derived capacity
                // (movement/manipulation) calculations. Reported for both animals and mechs.
