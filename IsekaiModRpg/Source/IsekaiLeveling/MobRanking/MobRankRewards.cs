...
namespace IsekaiLeveling.MobRanking
{
    ...
    public static class MobRankRewards
    {
        ...
        private static void SpawnManaCoreDrops(Pawn victim, MobRankComponent rankComp, Map map)
        {
            ...
            // Respect the master toggle — previously drops fired even when disabled.
            if (!IsekaiLevelingSettings.enableManaCoreDrops) return;
            ...
            // Forbid the dropped core unless the player opted into auto-hauling. Without this,
            // cores always spawned haulable regardless of the setting — including from
            // predator/animal kills the player never engaged (#332).
            if (!IsekaiLevelingSettings.autoAllowCrystalHauling)
            {
                manaCore.SetForbidden(true, false);
            }
