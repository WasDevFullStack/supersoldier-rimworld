...
namespace IsekaiLeveling.Patches
{
    ...
    public static class Patch_CombatXP_Melee
    {
        ...
        public static void Postfix(Pawn __instance, ref DamageInfo dinfo)
        {
            ...
            // Don't award per-hit combat XP for striking an already-downed/incapacitated
            // target. Prevents the infinite-XP exploit of meleeing a downed-but-unkillable
            // mechanoid forever (#342). Kill XP (Patch_KillXP) still fires if it dies, and
            // DamageTracker above still records the contribution.
            if (__instance.Downed) return;

            // Small XP for dealing damage. The damage used for XP is CAPPED so a single huge
            // hit can't grant explosive XP — e.g. Melee Animations executions / finishing moves
            // (#320), modded high-damage abilities like RoM Blade Dancer (#325), and raid
            // burst damage (#348) that previously scaled linearly with damage with no ceiling.
            float dmgForXp = Mathf.Min(dinfo.Amount, 50f);
            int xpAmount = Mathf.Min((int)(dmgForXp * 0.3f), 15);
