...
namespace IsekaiLeveling.Stats
{
    ...
    public class StatPart_IsekaiRestRate : StatPart_IsekaiBase
    {
        protected override void TransformValueCore(StatRequest req, ref float val)
        {
            ...
            float vit = GetStatValue(comp, "VIT");
            // Each VIT point = +X% rest rate (configurable via VIT_RestRate; default 1.5%).
            // Players can lower this to stop high-VIT pawns sleeping near-instantly.
            float multiplier = StatToMultiplier(vit, IsekaiLevelingSettings.VIT_RestRate);
