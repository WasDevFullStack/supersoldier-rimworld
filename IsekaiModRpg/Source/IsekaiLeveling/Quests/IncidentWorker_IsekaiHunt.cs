...
namespace IsekaiLeveling.Quests
{
    ...
    public class IncidentWorker_IsekaiHunt : IncidentWorker
    {
        ...
        public static void CreateHuntQuest(PawnKindDef creatureKind, QuestRank rank, float xpReward, float silverReward, Map homeMap, bool isBounty = false)
        {
            ...
            {
                // Local hunt (F-D by design, or a B+ hunt that fell back to local because no
                // world tile was found). MUST use the distance-free "_Hunt" description: the
                // rank-flavored "_Expedition"/"_Raid" strings contain a {4} (distance) token,
                // but the local path has no distance and passes only 4 args — which crashes with
                // "Could not resolve symbol 4". A local hunt summons the creature to the home
                // map (no travel), so the Hunt-contract wording is also the correct fit.
                quest.description = "Isekai_Quest_Description_Hunt".Translate(
                    rank.ToString(),
                    creatureKind.LabelCap,
                    NumberFormatting.FormatNum(xpReward),
                    NumberFormatting.FormatNum(silverReward)
        ...
        /// <summary>
        /// Check if a PawnKindDef's source mod passes the user's quest creature whitelist.
        /// Configured in Mod Settings → Quests → Quest Creature Mod Whitelist.
        /// When the whitelist is EMPTY, all mods pass (no restriction). When non-empty, ONLY
        /// creatures whose mod's package ID or name matches one of the comma-separated keywords
        /// (case-insensitive) are eligible. To also allow vanilla creatures, add "Core" or
        /// "ludeon" to the list.
        /// </summary>
        private static bool IsUserWhitelistedMod(PawnKindDef pk)
        {
            string whitelist = IsekaiMod.Settings?.QuestCreatureModWhitelist;
            if (string.IsNullOrEmpty(whitelist)) return true; // no whitelist = allow everything
            ...
            return false; // whitelist set but this creature's mod isn't on it
        }
        ...
        public static PawnKindDef SelectHostilePawnForRank(QuestRank rank)
        {
            ...
            // Apply the user's quest creature mod whitelist (when set, bounties only target
            // pawns from the whitelisted mods). Mirrors the hunt creature filter.
            pawnKinds.RemoveWhere(pk => !IsUserWhitelistedMod(pk));
            if (!pawnKinds.Any())
            {
                Log.Warning("[Isekai Hunt] No hostile humanlike pawn kinds passed the quest creature whitelist");
                return null;
            }
